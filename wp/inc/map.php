<?php
    ...
    function papj_customize_register( $wp_customize ) {
        $wp_customize->add_section( 'papj_map_section', array(
                'title' => __( 'Map', 'papillon-journey' ),
                'priority' => PAPJ_MAP_CUSTOMIZER_LOCATION
            ) );
        $wp_customize->add_setting( PAPJ_MAP_ID_SETTING, array(
                'default' => PAPJ_DEFAULT_MAP_ID,
                'sanitize_callback' => function ( $input ) {
                    return wp_get_attachment_image_url( $input, 'full' )? $input: PAPJ_DEFAULT_MAP_ID;
                }
            ) );
        $wp_customize->add_control( new WP_Customize_Media_Control( $wp_customize, PAPJ_MAP_ID_SETTING, array(
                'label' => __( 'Map Image', 'papillon-journey' ),
                'section' => 'papj_map_section',
                'mime_type' => 'image'
            ) ) );
        ...
        if ( ! is_dir( get_template_directory() . PAPJ_DEFAULT_MAP_TILES_PATH ) ):
            //TODO Do this on theme load?
            papj_generate_tiles_from_map_image( get_template_directory() . PAPJ_DEFAULT_MAP_PATH, get_template_directory() . PAPJ_DEFAULT_MAP_TILES_PATH );
            file_put_contents( get_template_directory() . '/log.txt', PHP_EOL . 'Init: done', FILE_APPEND );
        endif;
    }

    function papj_on_map_id_updated( $new_id, $old_id ) {
        ...
        try {
            if ( PAPJ_DEFAULT_MAP_ID !== $new_id ):
                $map_path = papj_download_map_image( wp_get_attachment_image_url( $new_id, 'full' ) );
                papj_generate_tiles_from_map_image( $map_path, get_template_directory() . PAPJ_MAP_TILES_PATH );
                unlink( $map_path );
            endif;
            return $new_id;
    ...
    function papj_download_map_image( $map_url ) {
        $map_path = get_template_directory() . PAPJ_MAP_PATH . '/' . PAPJ_DOWNLOADED_MAP_PREFIX . basename( parse_url( $map_url, PHP_URL_PATH ) );
    ...
    function papj_generate_tiles_from_map_image( $map_path, $tiles_directory ) {
        if ( file_exists( $tiles_directory ) ):
            papj_remove_file_or_directory( $tiles_directory );
        endif;
        mkdir( $tiles_directory );
        $tiler = new MapTiles( $map_path, array(
                'tiles_path' => $tiles_directory,
                'zoom_max' => 4
            ) );
        $tiler->process( true );
    }

    function papj_remove_file_or_directory( $path ) {
